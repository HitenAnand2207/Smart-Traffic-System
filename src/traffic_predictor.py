...
class TrafficPredictor:
    ...
    def update(self, vehicle_count: int, average_speed: float, congestion_level: float, timestamp: float):
        """
    ...
    def _exponential_smoothing(self, data: np.ndarray, alpha: float = 0.3) -> Dict:
        ...
        """
        if len(data) < 3:
            return {'current': float(data[-1]), 'predictions': [float(data[-1])] * self.prediction_horizon}
